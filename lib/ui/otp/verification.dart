...
class VerificationScreen extends StatefulWidget {
  ...
  VerificationScreen({Key? key, this.phoneNumber = '', required this.location})
      : super(key: key);
...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  verifyOtp(int mobile, String otp, int type) async {
    setState(() {
      isApiCalled = true;
    });
    Map<String, dynamic> map = {"mobile_no": mobile, "otp": otp, "type": type};
    print("requestParam ${map}");
    bool isOnline = await ApiClient.hasNetwork();
    if (isOnline) {
      await ApiClient.getClient().apiOtp(map).then((it) {
        if (it.status == "SUCCESS") {
          print("real data ${it.message}");
          print("real data ${it.data?.reqData.toString()}");
          print("real data ${it.data?.reqKey.toString()}");
          ...
          widget.location == "signup"
              ? logSignup(mobile.toString())
              : logLogin(mobile.toString());
  ...
  logLogin(String mobile) async {
    await FirebaseAnalytics.instance.logLogin(loginMethod: mobile);
  }

  logSignup(String mobile) async {
    await FirebaseAnalytics.instance.logSignUp(signUpMethod: mobile);
  }
