...
import 'package:revuer/networking/models/get_profile_model.dart';
import 'dart:convert';
import 'dart:io';
...
class SavePersonalInfoScreen extends StatefulWidget {
  const SavePersonalInfoScreen({Key? key}) : super(key: key);

  @override
  State<SavePersonalInfoScreen> createState() => _SavePersonalInfoScreenState();
}

class _SavePersonalInfoScreenState extends State<SavePersonalInfoScreen> {
  ...
  apiGetState() async {
    ...
    if (isOnline) {
      ApiClient.getClient().apiState().then((it) {
        if (it.status == "SUCCESS") {
          ...
          if (stateCityCheck) {
            if (getProfileModel.stateId != null) {
              ...
              _selectedstate = state;
              _state = state;
              print("selected before${_selectedstate}");
              print("selected before state list ${stateList.length}");
            }
          } else {
            if (stateId != "") {
        ...
        print("responseFailure get state ${obj}");
  ...
  apiGetCity(String stateId) async {
    ...
    if (isOnline) {
      ApiClient.getClient().apiCity(map).then((it) {
            ...
            if (getProfileCalled == true) {
              ...
              if (stateCityCheck) {
                if (getProfileModel.cityId != null) {
                  ...
                  _selectedcity = city;
                  _city = city;
                  print("selected before ${_selectedcity}");
                  print("selected before city list ${cityList.length}");
                }
              } else {
                if (cityId != "") {
        ...
        Fluttertoast.showToast(msg: "Something went wrong");
        // non-200 error goes here.
        print("responseFailure get city ${obj}");
        switch (obj.runtimeType) {
          case DioError:
            // Here's the sample to get the failed response error code and message
            final res = (obj as DioError).response;
  ...
  getProfileViaApi() async {
    ...
    print("requestParam save profile${map}");
    bool isOnline = await ApiClient.hasNetwork();
    if (isOnline) {
      ApiClient.getClient().apiGetProfile(map).then((it) {
        ...
        Fluttertoast.showToast(msg: "Something went wrong");
        // non-200 error goes here.
        print("responseFailure get profile ${obj}");
        switch (obj.runtimeType) {
          case DioError:
            // Here's the sample to get the failed response error code and message
            final res = (obj as DioError).response;
      ...
      await ApiClient.getClient()
        ...
        if (it.status == "SUCCESS") {
          ...
          Navigator.push(context, MaterialPageRoute(builder: (context) {
            ...
            /* print("then called");
            print("selected city before $_city");
            print("selected state before $_state");
            _state = "";
            _city = "";
            print("selected city after $_city");
            print("selected state after $_state");
            statesKey.currentState?.setSelectedItem(null);
            citiesKey.currentState?.setSelectedItem(null);*/
  ...
  userLogout() async {
    Map<String, dynamic> map = {
      "revuer_token": await SharedPrefProvider.getString(
          SharedPrefProvider.uniqueToken), //from share preference
      "player_id": await SharedPrefProvider.getString(
          SharedPrefProvider.playerId)
    };
    bool isOnline = await ApiClient.hasNetwork();
    if (isOnline) {
      ApiClient.getClient().apiUserLogout(map).then((it) {
        if (it["status"] == "SUCCESS") {
          SharedPrefProvider
              .clearPreference();
  ...
  void updateProfileViaApi(
    ...
    Map<String, dynamic> map = {
      ...
      "children": children,
      "mobile_no":
          phoneNoController.text.trim().toString(), //from share preference
    };
    print("requestParam ${map}");
    bool isOnline = await ApiClient.hasNetwork();
    if (isOnline) {
